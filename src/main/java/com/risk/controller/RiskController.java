...
public class RiskController {
    ...
    // ✅ GET /all — paginated + sorting (ANY logged-in user)
    @GetMapping("/all")
    ...
    public ResponseEntity<Risk> updateRisk(@PathVariable Long id,
                                           @Valid @RequestBody Risk updatedRisk) {
    ...
    // ✅ GET /export — ADMIN + MANAGER only (CSV download)
    @GetMapping("/export")
    @PreAuthorize("hasAuthority('ADMIN') or hasAuthority('MANAGER')")
    ...
    // ✅ Helper method to handle nulls & commas
